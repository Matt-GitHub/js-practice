...
function temperatureCtoF(temp) {
  let celsius = Math.round((temp * 9) / 5 + 32);
  return celsius;
}
...
function isItAnApple(array) {
  // create an empty array to capture result from for loop
  const returnedArray = [];
  for (let i = 0; i < array.length; i++) {
    if (array[i] === "apple") {
...
function getLastCarInfo(array) {
  const arrayMinusOne = array.length - 1;
  return `This is a ${array[arrayMinusOne].car_make} ${array[arrayMinusOne].car_model}`;
}
...
function getModelYears(array) {
  const carYearArray = [];
  for (let i = 0; i < array.length; i++) {
    carYearArray.push(array[i].car_year);
  }
  return carYearArray;
}
...
function getCarInfoById(array, number) {
  return `This is a ${array[number - 1].car_make} ${
    array[number - 1].car_model
  }`;
}
 ...
 */
function getOlderCars(array, max) {
  const desiredCarYears = [];
  for (let i = 0; i < array.length; i++) {
    if (array[i].car_year <= max) {
      desiredCarYears.push(array[i]);
    }
  }
  return desiredCarYears;
}
